//
//  ExchangeRateComparisonViewModel.swift
...
struct RateTrendViewModel: ViewModelProtocol {
    ...
    // ===== 새 데이터를 비교하고 상태를 업데이트하는 메서드 =====
    static func compareAndUpdateRates(newRates: [RateTrendViewModel]) -> [RateTrendViewModel] {
        ...
        saveNewRatesToCoreData(newRates: newRates)
        return updatedViewModels
    }

    // ===== Core Data에 새 데이터를 저장하는 메서드 =====
    static func saveNewRatesToCoreData(newRates: [RateTrendViewModel]) {
        newRates.forEach { rate in
            _ = CoreDataService.shared.savePreviousExchangeRate(currencyCode: rate.currency, rate: rate.rate)
        }
    }
}
